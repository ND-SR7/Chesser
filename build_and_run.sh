...
./mvnw spring-boot:run &> ../logs/server.log &
SERVER_PID=$!
if [ $? -eq 0 ]; then
  log_message "Server process started successfully (PID: $SERVER_PID). Waiting for Spring Boot to initialize..."
...
CLIENT_PID=$!
if [ $? -eq 0 ]; then
  log_message "Client process started successfully (PID: $CLIENT_PID). Waiting for the page to load..."
